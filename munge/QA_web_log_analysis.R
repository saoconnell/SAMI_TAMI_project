...
dataDir <- (paste(rootDir, TEST_TYPE, "/weblogs/", sep=''))
...
for (f in 1:length(test_files)) {
  ...
  if (grepl('log$', test_files[f])) {
    ...
    temp <- cbind(source=strsplit(test_files[f], "_")[[1]][1], temp)
...
header <- c("source","date","time","s.ip","cs.method","cs.uri.stem","cs.uri.query","s.port","cs.username","c.ip","cs.user.agent","sc.status","sc.substatus","sc.win32.status","time.taken")
...
setwd(paste("C:/Users/SAOb/Documents/workspace/RStudio_Projects/SAMI_TAMI/report_data/", sep="_"))
...
###  ******  THIS WILL NEED TO CHANGE IF PROCESSING ANY OTHER LOGS
###  ******  THIS WILL NEED TO CHANGE IF PROCESSING ANY OTHER LOGS
listTranNames <- (subset(top.url, Freq > 10000))
...
#load(file=(paste0(dataDir, "all_IIS_data.RData")))
...
for (top in 1:length(top30.p90.urls)) {
  openGdevice(type=output, file=paste("C:/Users/SAOb/Documents/workspace/RStudio_Projects/SAMI_TAMI/graphics/",  "top30_urls_DENSITY",  top, gsub("/", "_", top30.p90.urls[top]), sep="_"), w=10, h=10, r=108, ps=14)
...
as.data.frame(table(all_IIS_data$uname))
sink()
options(width=80)
...
cat("\n\nUNIQUE URLs:\n")
as.data.frame(table(all_IIS_data$stem))
sink()
options(width=80)
...
topD <- d_ply(all_ss, .(stem), function(df) {
  ...
  openGdevice(type=output, file=paste("C:/Users/SAOb/Documents/workspace/RStudio_Projects/SAMI_TAMI/graphics/",  "_iis_by_user_count",  gsub("/", "_", mtrc), sep="_"), w=10, h=10, r=108, ps=14)
  
  print(bwplot(time.taken ~ user_count, data=df, 
               type='l', 
               #layout=c(1,1),
               as.table=TRUE,
               #strip=FALSE,
...
#ss <- subset(all_ss, Entity %in% esx.hosts)
all_ss$stem <- as.factor(as.character(all_ss$stem))
topD <- d_ply(all_ss, .(test), function(df) {
  
  mtrc <- as.character(df$test[1])
  
  df <- sort.data.frame(df,~+POSIX)
  
  openGdevice(type=output, file=paste("C:/Users/SAOb/Documents/workspace/RStudio_Projects/SAMI_TAMI/graphics/QA_weblog_graphics/",  "_transaction",  by_break, mtrc, sep="_"), w=10, h=10, r=108, ps=14)
  ...
  num_users <- as.numeric(strsplit(strsplit(mtrc, "\\_")[[1]][2], "U")[[1]][1])
  tx_user   <- y_mean / num_users
  
  #cat(mtrc, " mean ", mean(y), " other mean ", y_mean, "tx/user ", tx_user, "\n")
